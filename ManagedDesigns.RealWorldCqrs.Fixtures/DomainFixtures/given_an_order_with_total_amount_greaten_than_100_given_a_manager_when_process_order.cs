...
{
    [TestClass]
    public class given_an_order_with_total_amount_greaten_than_100_given_a_manager_when_process_order
        : AggregateFixtureBase
    {
        ...
        [TestMethod]
        public void then_order_validated_by_manager()
        {
            this.events.Count().Should().Be.EqualTo(1);
            this.events.Single().Should().Be.OfType<OrderValidatedByManager>();
        }
    }
}